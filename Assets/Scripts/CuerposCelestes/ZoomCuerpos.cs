...
public class ZoomCuerpos : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (dir.y < -limite || dir.y > limite)
        {
            ...
            transform.position = objetivo.position + dir.normalized * distanciaActual;
        }
    ...
    void ManejarDrag()
    {
        ...
        float limite = 0.8f;
        float deltaY = -t.deltaPosition.y * dragSpeed;
        Quaternion rotSimulada = Quaternion.AngleAxis(deltaY, transform.right);
        Vector3 posSimulada = objetivo.position + rotSimulada * (transform.position - objetivo.position);
        Vector3 dirSimulada = (posSimulada - objetivo.position).normalized;

        if (dirSimulada.y >= -limite && dirSimulada.y <= limite)
            transform.RotateAround(objetivo.position, transform.right, deltaY);
